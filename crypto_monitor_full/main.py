...
# 480 close/volume по каждому ТФ
...
async def prefill_history():
    """480 баров close+volume по всем ТФ и символам."""
...
async def updater_loop():
    """Раз в 30 сек обновляем цены и при закрытии свечи добавляем новый close/volume."""
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            now = time.time()
            ready = [tf for tf, sec in TIMEFRAMES.items()
            ...
            if tickers:
                ...
                for sym in symbols_top25:
                    ...
                    for tf in ready:
                        ...
                        # объём – просто повторяем последний известный, для демо достаточно
